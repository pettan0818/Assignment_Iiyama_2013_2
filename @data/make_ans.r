...
if(with_test_flag == 0){
	...
	# 今と昔が一緒.0の自動設定
	if (input_data["pref_u15"] == input_data["pref_now"]) input_data["equivalent"] = 0 else input_data["equivalent"] = 1
}
print(input_data)
...
if(with_test_flag == 0){
	result = testing_Algo(input_data, data, 8)
}
...
if (with_test_flag == 0){
	names(result) = c("shrimp","conger_eel","tuna","squid","sea_urchin","salmon_roe","egg","fatty flesh","tuna_roll","cucumber")
	#print(sort(names(result)))
	print(sort(result)[1:3])
}