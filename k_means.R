#データの読み込み
ua <- read.csv("C:/Users/kazu/Documents/Test/Assignment_Iiyama_2013_2/@data/udata+aorder_labeled_work.csv")
id <- c(9999,0,1,90,7,2,0,25,6,1,1)
#クラスタリングと結合
udata.r <- rbind(udata,id)
udata.r <- udata.r[2:]
udata.k <- kmeans(udata.r,8)
udata.t <- transform(ua,cluster=udata.k$cluster[1:5000])
#同クラスタの人を抽出
